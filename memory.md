...
- **Testimonial Section:** Use a public API endpoint (e.g., `https://dummyjson.com/quotes`) with Next.js ISR (`next: { revalidate: 3600 }`) for caching. *(Implemented with the project's own public endpoint `/api/testimonials`, see §17, so the data matches the design exactly.)*
...
- Breadcrumb "Home / Custom Jewelry" 14px lh 160%; "Home" and "/" `muted` (#757575, was Figma #7A7A7A), current page `ink`. Tokens added: `heading` #101010, `muted` #7A7A7A.
...
- **Data (project rule, revised):** the testimonials come from the project's own public endpoint `GET /api/testimonials` (`app/api/testimonials/route.ts`, `export const revalidate = 3600` + `Cache-Control: public, s-maxage=3600, stale-while-revalidate=86400`, CORS `*`). The data lives in `lib/testimonials.ts` and matches the Figma cards exactly (4 people, role "Product Quality Engineer", the four quotes verbatim incl. the double space in the 4th, rendered as space + nbsp because HTML collapses it); cards 5–12 are same-style placeholders so there are 3 pages/dots. `getTestimonials()` does `fetch(`${API_ORIGIN}/api/testimonials`, { next: { revalidate: 3600 } })` (dev → `http://localhost:$PORT|3000`, otherwise `SITE_URL`) and falls back to the same in-repo list with a `console.warn` if the request fails — this is what happens during a build before the endpoint exists on the live site. The earlier dummyjson source was dropped because its random quotes could not match the design.
...
- `lib/site.ts` holds `SITE_URL` (`NEXT_PUBLIC_SITE_URL`, else `VERCEL_PROJECT_PRODUCTION_URL`, else `LIVE_URL` = `https://my-jewel-mauve.vercel.app`, the deployed site; set `NEXT_PUBLIC_SITE_URL` only for a custom domain), title/description, the OG image and the `ROUTES` list used by the sitemap.
...
- **No pop-in on reload:** `next/image` lazy-loads by default and `public/` files are revalidated on every visit, so small UI icons/badges appeared late (flicker). Header + footer social icons are inlined SVG via `components/InlineIcon.tsx` (server component, reads `public/icons/*.svg` at build time; `className` gives a responsive box). Footer payment PNGs and the newsletter send icon use `loading="eager"` (+ `decoding="sync"`, `fetchPriority="low"` for the PNGs). `next.config.ts` sets `Cache-Control: public, max-age=86400, stale-while-revalidate=604800` on `/icons/*` and `/images/*`. Large below-the-fold photos stay lazy.
- **Lighthouse pass (mobile, local prod build):** `experimental.inlineCss` in `next.config.ts` (removes the render-blocking stylesheet request), Open Sans `preload: false` (desktop-only label, off the mobile LCP path), breadcrumb "Home" link is `inline-block` (fixes axe `link-in-text-block` with no visual change). A11y 89 → 93. Verified UI unchanged with a full-page pixel diff at 1440 and 390 (0 differing pixels). **`muted` token is now `#757575`** (Figma `#7A7A7A` is 4.29:1 on white, AA needs 4.5:1; the 5-level difference is not visible). **Still failing, needs a visible change:** `target-size` (testimonial dots 8px at 18px pitch, mobile footer social icons 18px; overlapping bigger hit areas don't satisfy axe, only wider spacing does).
